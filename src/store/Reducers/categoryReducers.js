...
export default (state = initialData, action) => {
  ...
  switch (action.type) {
    case "persist/REHYDRATE":
      action.payload &&
        (newState.categories = action.payload.categoryReducers.categories);
      break;
    ...
    case "CATEGORY_DELETED":
      let newproduct = Object.assign({}, newState, {
        categories: newState.categories.filter(product => {
          console.log(action.payload);
          return product._id != action.payload._id;
        })
      });
      return newproduct;
    case "CATEGORYIES_CREATED":
      newState.categories = [...newState.categories, ...action.payload];
      break;
    case "CATEGORIES_LOADED":
      return Object.assign({}, newState, {
        categories: (newState.categories = action.payload)
      });
  ...
  // newState.categCode =  "CAT" + newState.categories.lenght;
  newState.categCode = utlities.getID("C", "code", newState.categories);
  return newState;
};
