          ...
          fetch("/api/categories/upload", {
            ...
            .then(categories => {
              if (categories.length > 0) {
                dispatch({
                  type:  "CATEGORYIES_CREATED",
                  payload: categories,
                  model: "categories"
                });
                toast.success(messages.categories[data._id ? "updated" : "saved"]);
              } else {
                toast.error(messages.category.error.cannotCreate);
              }
            });
        };
      },
          ...
          fetch("/api/categories/delete/" + id, {
            ...
            .then(sale => {
              if (sale._id) {
                ...
                toast.error(messages.categories.deleted);
              } else {
