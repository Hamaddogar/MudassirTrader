...
export default {
  ...
  deleteCustomer(data) {
    console.log(data);
    return dispatch => {
      fetch("/api/customers/deleteCustomer" + data, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json"
        }
        ...
        .then(customer => {
          if (customer._id) {
            ...
            toast.error(messages.customers.deleted);
          } else {
            toast.error(messages.customers.error.cannotCreate);
          }
        });
  ...
  uploadCsutomers(data) {
    return dispatch => {
      fetch("/api/customers/upload", {
        ...
        .then(customers => {
          console.log("middle", customers);
          if (customers.length > 0)  {
            dispatch({
              type: "CUSTOMERS_CREATED",
              payload: customers,
              model: "customers"
            });
