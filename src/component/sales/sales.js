...
class Sales extends React.Component {
  ...
  updateState = evt => {
    ...
    if (evt.target.id == "sale") {
      let newResult = this.props.data.sales.filter(sale => {
        return this.state.sale.billNo == sale.billNo;
      });
      this.setState({ sales: newResult });
    } else if (evt.target.id == "area") {
      let newResult = this.props.data.sales.filter(sale => {
        return this.state.area.name == sale.area.name;
      });
      this.setState({ sales: newResult });
    } else if (evt.target.id == "customer") {
      let newResult = this.props.data.sales.filter(sale => {
        return this.state.customer.firstName == sale.customer.firstName;
      });
      this.setState({ sales: newResult });
    }
  };
