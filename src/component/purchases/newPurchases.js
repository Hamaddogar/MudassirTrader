...
import  PurchasePrint from './purchesPrint/purchasePrint'
...
class purchases extends React.Component {
    ...
    this.props.data.purchases.forEach(purchase => {
      ...
      if (purchase._id == id) {
        ...
        this.setState({
          ...
          deliveryDate: new Date(purchase.deliveryDate).toDateString(),
          salesDate: purchase.salesDate ? new Date(purchase.salesDate).toDateString():null,
  ...
  savePurchase = type => {
    ...
    //  {
    //    alert("please Slect Supplier")

    //  }

    //   else if( Object.keys(this.state.area).length===0)
    //   {
    // alert("please Slectr Area")
    ...
    let products = this.state.bills.map(bill => {
      return bill.product._id;
    });
    console.log(products);
    setTimeout(() => this.props.loadProduct(products), 200);
      ...
      <section className="app-section new-purchase">
        ...
        <div className="label-head">
          ...
          <div className="text-right right-panel">
            ...
            <img
              onClick={() => {
                this.savePurchase("saveContinue");
                this.setState({
                  deliveryDate: null,
                  salesDate: new Date(),
                  deleteDialog: false,
            ...
            <div style={{ display: "none" }}>
              ...
              <PurchasePrint   store ={this.props.data.storedata} totalBill = {Math.round(totalBill)}  totalItemDiscount={Math.round(totalDiscount)}  totalUnitDiscount={Math.round(totalUnitDiscount)} 
              
              
          totalpayable=  {Math.round(totalPyablePayment) > 0
                ? Math.round(totalPyablePayment)
                : 0}
              data ={this.state}  supplierdata={this.props.data} ref={el => (this.componentRef = el)} />
        ...
        <div className="row">
          <div class="input-field col s3">
            <AutoComplete
              ...
              value={this.state.supplier.firstName}
            />
          ...
          <div class="input-field col s3">
            <input
              ...
              ref="salesDatePicker"
              type="text"
              value={this.state.salesDate ? new Date(this.state.salesDate).toDateString() : null}
            />
          </div>
        </div>
        ...
        <div className="row">
          <div class="input-field col s3">
            <input
              ...
              value={this.state.deliveryDate ?new Date(this.state.deliveryDate).toDateString() : null}
            />
          </div>
